<template>
  <div
    class="reveal-on-scroll overflow-hidden group relative"
    :class="[featured ? 'featured-card' : 'glass-card', { 'cursor-pointer': project.url }]"
    @click="openProject"
  >
    <!-- 特色卡片布局（前三个项目） -->
    <template v-if="featured">
      ...
      <div
        v-else
        class="absolute inset-0 bg-gradient-to-br from-primary-pink/30 to-primary-purple/30"
      >
    ...
    <template v-else>
      ...
      <div
        v-if="project.cover"
        class="h-48 bg-gradient-to-br from-primary-light to-purple-light overflow-hidden"
      >
        <img
          :src="project.cover"
          :alt="project.name"
      ...
      <div
        v-else
        class="h-32 bg-gradient-to-br from-primary-pink/20 to-primary-purple/20 flex items-center justify-center"
      >
        <svg
          class="w-8 h-8 text-primary-purple/40"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <div class="p-5 flex-1 flex flex-col">
        ...
        <div class="flex items-start justify-between mb-3 gap-2">
          <h3
            class="text-lg font-semibold text-text-primary leading-tight group-hover:text-gradient transition-all duration-300"
          >
            {{ project.name }}
          </h3>
          <span
        ...
        <!-- 项目描述 -->
        <p class="text-text-body text-sm mb-4 line-clamp-2 leading-relaxed flex-1 font-normal">
          {{ project.description }}
        </p>
        ...
        <!-- 版本和更新时间 - 固定在底部 -->
        <div
          class="flex items-center justify-between text-xs text-text-secondary pt-3 border-t border-gray-100 mt-auto font-normal"
        >
      ...
      >
        <div
          class="absolute -top-20 -right-20 w-40 h-40 bg-primary-pink/10 rounded-full blur-3xl"
        ></div>
        <div
          class="absolute -bottom-20 -left-20 w-40 h-40 bg-primary-purple/10 rounded-full blur-3xl"
        ></div>
      </div>
    </template>
  </div>
...
const statusClass = computed(() => {
  const statusMap = {
    开发中:
      'bg-gradient-to-r from-primary-pink/20 to-primary-pink/10 text-primary-dark border border-primary-pink/30',
    已上线:
      'bg-gradient-to-r from-primary-purple/20 to-primary-purple/10 text-primary-purple border border-primary-purple/30',
