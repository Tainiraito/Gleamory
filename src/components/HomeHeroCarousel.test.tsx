...
describe('HomeHeroCarousel', () => {
  it('只展示站点标题与简介，不再展示项目元信息', () => {
    render(<HomeHeroCarousel title="微光集" description="汇聚所有微光般的创意与灵感" />)

    const carousel = screen.getByRole('region', { name: '微光集图片轮播' })
    expect(carousel).toHaveClass('w-full', 'bg-[var(--bg-card)]')
    ...
    expect(
      within(within(carousel).getByRole('img').parentElement!).queryByRole('button'),
  ...
  it('可以通过按钮和键盘切换图片', () => {
    ...
    expect(within(carousel).getByAltText('桑多涅与小动物在暖色光影中的插画')).toBeInTheDocument()

    fireEvent.click(within(carousel).getByRole('button', { name: '暂停自动轮播' }))
    expect(within(carousel).getByRole('button', { name: '继续自动轮播' })).toBeInTheDocument()
