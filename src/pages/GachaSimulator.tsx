    ...
    if (raw) {
      ...
      const entries = Array.isArray(parsed.entries)
        ? parsed.entries.filter(
            (e: unknown) => e != null && typeof (e as Entry).name === 'string'
          )
        : []
...
// Compute per-card probability and per-unique probability
function entryProb(name: string, entries: Entry[], totalCards: number) {
  const cardCount = entries.filter((e) => e.name === name).length
  const uniqueNames = [...new Set(entries.map((e) => e.name))]
  const uniqueCount = uniqueNames.length
  const cardPct = ((cardCount / totalCards) * 100).toFixed(1)
  const uniquePct = ((1 / uniqueCount) * 100).toFixed(1)
  return { cardPct, uniquePct, cardCount }
}
...
const GachaSimulator = () => {
  ...
  const handleAddEntries = useCallback(() => {
    ...
    if (entryMode === 'overwrite') {
      ...
      if (dedupEnabled) {
        ...
        const deduped = newEntries.filter((e) => {
          if (existing.has(e.name)) return false
          existing.add(e.name)
          return true
        })
  ...
  const handleRemoveEntry = useCallback((entryIdx: number) => {
    ...
    if (newEntries.length > 0) rebuild(newEntries)
  }, [entries, rebuild])
  ...
  return (
    <div className="relative min-h-screen" style={{ background: 'var(--bg-page)' }}>
      <div className="fixed top-6 sm:top-8 left-6 sm:left-8 z-50">
        <Link to="/" className="font-display text-[0.6rem] uppercase tracking-[0.3em] hover:opacity-70 transition-opacity duration-300" style={{ color: 'var(--text-muted)' }}>Gleamory</Link>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
        ...
        {/* PC: 3 columns | Mobile: stack */}
        <div className="flex flex-col lg:flex-row gap-6">
          {/* ===== Column 1: Management ===== */}
          <div className="w-full lg:w-64 shrink-0 space-y-4">
            <div className="p-5" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
              <h2 className="text-xs uppercase tracking-widest mb-3" style={{ color: 'var(--text-secondary)' }}>添加条目</h2>
              ...
              <div className="flex mb-2" style={{ border: '0.5px solid var(--border-line)' }}>
                <button onClick={() => setDedupEnabled(true)}
                  className="px-3 py-1.5 text-xs transition-all duration-200 flex-1"
                  style={{ background: dedupEnabled ? 'rgba(44,42,48,0.04)' : 'transparent', color: dedupEnabled ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                  去重
                </button>
                <button onClick={() => setDedupEnabled(false)}
                  className="px-3 py-1.5 text-xs transition-all duration-200 flex-1"
                  style={{ borderLeft: '0.5px solid var(--border-line)', background: !dedupEnabled ? 'rgba(44,42,48,0.04)' : 'transparent', color: !dedupEnabled ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                  不去重
                </button>
              </div>

              {/* Row 2: append/overwrite toggle */}
              <div className="flex mb-2" style={{ border: '0.5px solid var(--border-line)' }}>
                <button onClick={() => setEntryMode('append')}
                  className="px-3 py-1.5 text-xs transition-all duration-200 flex-1"
                  style={{ background: entryMode === 'append' ? 'rgba(44,42,48,0.04)' : 'transparent', color: entryMode === 'append' ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                  追加
                </button>
                <button onClick={() => setEntryMode('overwrite')}
                  className="px-3 py-1.5 text-xs transition-all duration-200 flex-1"
                  style={{ borderLeft: '0.5px solid var(--border-line)', background: entryMode === 'overwrite' ? 'rgba(44,42,48,0.04)' : 'transparent', color: entryMode === 'overwrite' ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                  覆盖
                </button>
              </div>
              ...
              {/* Progress bar */}
              <div className="mt-4 pt-4" style={{ borderTop: '0.5px solid var(--border-line)' }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[0.6rem]" style={{ color: 'var(--text-muted)' }}>全部 {totalCards} 张</span>
                  <span className="text-[0.6rem]" style={{ color: 'var(--accent-pink)' }}>已翻 {flippedCount}</span>
                </div>
                <div className="h-1 rounded-full" style={{ background: 'var(--border-line)' }}>
          ...
          {/* ===== Column 2: Remaining + Drawn Results (PC) ===== */}
          <div className="w-full lg:w-72 shrink-0 space-y-4">
            {/* Remaining entries */}
            <div className="p-5" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
              <h2 className="text-xs uppercase tracking-widest mb-3" style={{ color: 'var(--text-secondary)' }}>
                剩余 {totalCards - flippedCount} / {totalCards}
              </h2>
              <div className="max-h-64 overflow-y-auto space-y-0.5">
                {cardOrder.map((entryIdx, cardIdx) => {
                  if (flipped[cardIdx]) return null
                  const entry = entries[entryIdx]
                  const { cardPct, uniquePct } = entryProb(entry.name, entries, totalCards)
                  return (
                    <div key={cardIdx} className="flex items-center gap-1 py-0.5 group">
                      <span className="text-[0.6rem] flex-1 truncate" style={{ color: 'var(--text-muted)' }}>{entry.name}</span>
                      <span className="text-[0.5rem] shrink-0" style={{ color: 'var(--text-muted)' }}>{cardPct}%({uniquePct}%)</span>
            ...
            {history.length > 0 && (
              <div className="p-5" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
                ...
                {/* Stats */}
                <div className="mt-3 pt-3" style={{ borderTop: '0.5px solid var(--border-line)' }}>
                    ...
                    {Array.from(drawnStats.entries()).sort((a, b) => b[1] - a[1]).map(([name, count]) => {
                      ...
                      return (
                        <div key={name} className="flex items-center gap-2">
                          <span className="text-[0.6rem] w-16 truncate shrink-0" style={{ color: 'var(--text-primary)' }}>{name}</span>
            ...
            {history.length === 0 && (
              <div className="p-5 text-center" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
                <p className="text-[0.6rem]" style={{ color: 'var(--text-muted)' }}>✦ 点击卡牌开始抽卡</p>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            {entries.length === 0 ? (
              <div className="flex items-center justify-center py-32" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>✦ 添加条目开始抽卡</p>
              </div>
            ) : allFlipped ? (
              <div className="flex flex-col items-center justify-center py-32 gap-4" style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border-line)' }}>
                <p className="font-display text-lg" style={{ color: 'var(--text-primary)' }}>全部翻完啦 ✦</p>
                      ...
                      <motion.button key={cardIdx} onClick={() => handleCardClick(cardIdx)}
                        ...
                        <motion.div
                          ...
                          <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', background: 'var(--bg-elevated)', border: '0.5px solid var(--border-line)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '6px' }}>
                            <span className="block font-display text-xs font-semibold text-center leading-tight mb-0.5" style={{ color: 'var(--text-primary)', wordBreak: 'break-word' }}>{entry.name}</span>
                            {isFlipped && (
                              <span className="text-[0.5rem]" style={{ color: 'var(--accent-pink)' }}>
                                {cardPct}%({uniquePct}%)
                              </span>
                            )}
                          </div>
      ...
      {/* Mobile: Results below cards (only shown on mobile via lg:hidden) */}
      {history.length > 0 && (
      ...
      <footer className="flex flex-col items-center gap-3 pt-8 pb-12">
        ...
        <Link to="/" className="text-[0.6rem] uppercase tracking-widest transition-opacity duration-300 hover:opacity-70" style={{ color: 'var(--text-muted)' }}>&larr; 返回首页</Link>
      </footer>
    </div>
  )
}
