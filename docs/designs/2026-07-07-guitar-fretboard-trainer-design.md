...
| P0 | 认音题入口 | 当前页面以找音题为主，需补“给位置回答音名”的完整交互 |
...
2. 首版不再提供独立“记录”Tab。正确率、平均反应、薄弱区域和实时反馈保留在侧栏；长期记录、热力图、错题回看仍作为后续扩展。
...
4. 自选题目参数改变后不会隐式提交或计入统计，用户点击“生成题目”后才开始新题并重新计时。
...
1. 切换出题方式、题型或目标参数只更新待生成配置，不自动提交当前题，也不写入统计。
2. 点击“生成题目”后清空当前选择、临时音名、淡出计时器和答案反馈，生成新题并重新开始反应计时。
...
7. 当前侧栏继续以音名聚合薄弱项；更细标签先写入数据，为后续按题型、弦、把位、音程和音级分析保留兼容路径。
