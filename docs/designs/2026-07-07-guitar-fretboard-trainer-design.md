...
> **文档状态说明（2026-07-14）：** 本文前 21 章和第 22 章早期小节保留产品愿景与决策历史，不再作为当前版本的逐项验收清单。当前 `v0.3.0` 能力以 22.20-22.23、`2026-07-14-guitar-fretboard-single-question-stats-design.md`、`2026-07-14-guitar-fretboard-appearance-presets-design.md`、`docs/requirements.md` 和实际测试为准。每日记录、练习历史、热力图、学习路径、自定义调弦编辑和数据导入导出均不得描述为当前已支持。
...
本节是阅读本文时判断当前能力的首要入口；与前述历史章节冲突时，以本节和 22.20-22.22 为准。

1. 页面由顶部唯一指板和“今日练习”“指板地图”“设置”三个 Tab 组成，不存在独立测验、记录或学习路径 Tab。
2. 今日练习默认无题，手动生成后才开始后台计时；答题结束后展示冻结用时。单选题准确率为 0% 或 100%，多选题按答案集合与选择集合的交并比计分。
3. 当前支持找音、认音、八度、音程和调内音五类题型；随机混合可限制题型及关联参数，自选题目可主动指定题型、目标和品格范围。
4. 指板地图支持整体或单弦升降半音、把位筛选、A-G 指定音多选，以及大调、自然小调、小五声音阶和三类和弦分布。
5. 左键选择并播放采样、再次左键重复播放、右键取消；地图和设置可按配置隐藏或淡出音名，今日练习答案不自动消失。
6. 地图当前位置详情独立于视觉选择，音名淡出或右键取消后仍保留，点击下一位置时更新。
7. 设置保存调弦、四套指板外观、升降号偏好、基础显示模式和点击音名时长。指板使用递减品距、六弦独立粗细、缠弦/钢弦、琴枕、品丝及真实品位标记。
8. 术语解释悬停 500ms 后出现，键盘聚焦和点击立即打开；相关词条嵌套不设固定层数，仅通过访问路径阻止循环。
9. 本地存储只保存设置，不保存逐题结果、每日聚合、练习历史、热力图或技能状态。
10. 当前项目卡片版本为 `v0.3.0`，更新时间为 `2026-07-14`，发布状态仍为“开发中”。
