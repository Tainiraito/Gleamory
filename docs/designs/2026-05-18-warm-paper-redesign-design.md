...
- 普通项目按用户目标分为“音乐与声音、图像与内容、轻松一刻”，分类顺序和项目归属由独立展示配置维护，不改变 `projects.json` 主数据结构。
- 桌面端以紧凑的左侧分类栏配合右侧三列卡片网格，分类标题使用两位数字、宋体标题、简短说明和项目数量建立杂志目录层级；窄屏恢复为标题在上、卡片在下。
- 分类间距、标题区高度和普通卡片内边距保持紧凑，不再为补齐一行生成“敬请期待”占位卡。
- 新项目暂未写入分类配置时进入“其他项目”回退分组，避免卡片静默消失；元数据测试仍要求正式项目在提交前完成归类。
- 卡片进入视口和悬停动效遵循系统 `prefers-reduced-motion` 设置；减少动态效果时直接显示最终状态。
...
- 使用深墨色无圆角画框承载本地插画，图片保持原有色彩；只在标题所在左侧与底部增加局部暗部渐变，不做全图染色。
- 右上使用细线图标提供上一张、暂停/继续和下一张，页码采用 `01 / 03` 格式；底部使用可点击分段进度。
...
- 高度在移动端为 22rem、较宽屏幕为 24rem，标题与控制在窄屏保持可读且不遮挡主要人物。
