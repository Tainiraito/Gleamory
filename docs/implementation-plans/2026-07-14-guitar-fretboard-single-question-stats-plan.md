...
**Architecture:** 页面以 `QuizQuestion | null` 表达未生成、作答中和已完成状态；`quiz.ts` 提供可独立测试的单题准确率函数；`QuizPanel` 内部的轻量计时展示组件每 100 毫秒刷新，答题后使用 `responseMs` 冻结。存储层仅保留设置，旧历史字段在读取时忽略。
...
- Produces: 初始“生成题目”空状态、作答中实时用时、完成后冻结用时与准确率

- [ ] **Step 1: 为初始无题和生成行为写页面失败测试**

断言初始没有题干和提交按钮，显示“尚未生成题目”“生成题目”，指板不能选择；点击生成后出现题干、`用时 0.0 秒` 和 `准确率 --`。

- [ ] **Step 2: 为计时冻结和部分准确率写失败测试**

使用 fake timers 验证生成后时间增长，重置选择后时间不归零，提交后时间停止；构造固定找音题选择验证部分准确率。认音题继续验证点击选项即提交。

- [ ] **Step 3: 验证页面与组件测试正确失败**
...
- [ ] **Step 5: 在 QuizPanel 内实现局部计时展示**

`QuizPanel` 接受可空题目和 `questionStartedAt`。无题时渲染空状态；有题时每 100 毫秒只更新组件内部展示时间，答案存在时改用 `answer.responseMs`。准确率使用纯函数计算并显示整数百分比。

- [ ] **Step 6: 修正重置、提交和设置持久化**
