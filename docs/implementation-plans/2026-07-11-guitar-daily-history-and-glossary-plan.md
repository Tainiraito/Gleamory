...
当前阻塞：应用内浏览器在 WSL 工作区初始化时拒绝 `sandboxCwd` 的 `file:///home/...` 元数据，未进入页面自动化。用户将在当前浏览器中直接测试最终效果并继续反馈。
