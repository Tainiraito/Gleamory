...
仓库不保存 ONNX 权重。`src/lib/onnx/modelRegistry.ts` 是模型下载源的代码事实源，`public/models/README.md` 记录手动下载、校验和许可信息。修改其中一处时必须同步另一处。

ONNX Runtime WASM 由当前 `package-lock.json` 锁定的 `onnxruntime-web` 与 Vite 构建自动生成。不得在 `public/` 中人工复制另一套 WASM/MJS，也不得通过 `ort.env.wasm.wasmPaths` 指向未经版本校验的静态副本；否则会产生依赖与二进制版本错配，并让生产包携带重复资源。
...
- 可用的模型源应固定到不可漂移的版本或提交，并记录精确大小和校验值。
- 远程源需要考虑 CORS、重定向、Content-Type、Content-Length 和托管稳定性。
- 静态 `/models/...` 只能作为实际部署了对应资源时的来源，不能假设仓库中已有大模型。
- 不得在代码或公开文档中写入开发者绝对路径、局域网地址或个人代理端口。
- 下载源修复必须同时验证开发模式和生产构建路径，不能只验证 `npm run dev`。

如果远程资源不稳定，优先更换受控或明确支持跨域的托管源，不要降低模型内容校验标准。模型仓库未声明权重许可时，不得用上游代码许可证代替，也不应将权重重新分发到本仓库。
